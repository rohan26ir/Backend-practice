...
async function run() {
  try {
    ...
    app.put("/updateBlog/:id", async (req, res) => {
      ...
      const options = {upsert: true}; // if the document does not exist, it will be created
      const updateBlog = req.body; // get the updated data from the request body
      const blog = {
        $set: {
      ...
      const result = await practiceCollection.updateOne(query, blog, options);
      res.send(result);
    })
